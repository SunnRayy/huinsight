...
**Files**: `src/identity/authority_resolver.py`
...
| 6 | `shadow_mutual_exclusion` | Qty-bearing reader rows at the source's latest snapshot are not shadowed, except zero-qty co-authority tombstones and rows superseded by an active `Consolidated` row | 0 unsuperseded shadowed reader rows |
