...
| `ux-command-center/tests/import-adapters-panel.test.tsx` | 5 / 5 | Buttons the test looks for by name (Upload/Stage/Approve/Holdings/Transactions) aren't found — the component's UI was restructured after the test was written. |
| `ux-command-center/tests/audit-page.test.tsx` | 4 / 4 | Page heading/copy text no longer matches. |
| `ux-command-center/tests/performance-batch6.test.tsx` | 4 / 12 | CSS class assertions (`border-green-200` and similar) — styling drifted, not logic. |
| `ux-command-center/tests/taxonomy-pages.test.tsx` | 2 / 10 | Content text not found ("Market Price", a column header) — likely a label rename. |
| `ux-command-center/tests/routes-smoke.test.tsx` | 2 / 8 | Two pages' `data-testid` no longer matches — route or page-shell structure changed. |
| `components/ai-advisor/AssetAnalyzer.test.tsx` | 1 / 6 | Single disabled-state assertion mismatch. |
| `ux-command-center/tests/analytics-batch1.test.tsx` | 1 / 4 | Single content-text assertion mismatch. |
| `ux-command-center/tests/balance-sheet-api.test.ts` | 1 / 1 | API call made with different arguments than the test expects. |
| `ux-command-center/tests/dashboard-batch2.test.tsx` | 1 / 1 | Same shape — a mock's call arguments don't match. |
| `ux-command-center/tests/wealthos-regression.test.tsx` | 1 / 1 | Content text ("AAPL") not found. |
