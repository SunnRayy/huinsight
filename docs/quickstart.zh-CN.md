...
全新演示环境里有两项检查不会显示为通过，这两项都在预期之内：

- `unmatched_security_transfer`：在这份演示数据集上会显示为一条提示性发现——
  这是合成转账历史本身的一个已知的、无害的特征，不是你环境的 bug。
- `twr_in_range`：显示为**跳过**，而不是失败。它需要覆盖组合一半市值、且有
  满一年历史的资产；全新安装时券商和基金读取器的数据还不满一年。检查详情里
  会写明这一点，历史积累够了之后会自动开始检查。

如果只想在不跑完整同步的情况下拿到一份机器可读的重跑结果，
`--check-integrity --json` 同样可用。

### 写入演示用的 AI 内容

AI 顾问是这个产品的核心。它的内容（投资理念、策略备忘、简报、复盘和洞察）存放在
数据库里，而不在上面的源文件中。为演示人物写入这些内容：

```bash
.venv/bin/python tools/demo_data/seed_ai.py
```

它会写入一份投资者画像、预测页所用的退休目标、三份策略备忘、一份示例简报和一份示例复盘（在应用里都标注为
**示例**——为演示人物撰写，并非模型生成）、覆盖各生命周期阶段的洞察，以及几笔已核验
的交易。它只会写入包含演示人物的数据库，否则拒绝执行；`--dry-run` 可预览将写入的内容，
重复运行不会产生变化。
...
AI 顾问的各个页面（档案、备忘、洞察、示范简报和复盘、上下文预览）在没有 API 密钥的情况下也能工作。生成新的简报、复盘、分析和审计则需要后端环境中配置一个 LLM API 密钥。启动应用前，设置以下其中一个环境变量：
...
然后像往常一样启动应用：`./dev.sh start`。应用也会读取仓库根目录下的 `.env` 文件，所以你也可以把密钥存放在那里：

```bash
# 在仓库根目录创建 .env（已被 gitignore）
echo "GEMINI_API_KEY=your-key" > .env
```

没有密钥的情况下，AI 页面上的「生成」按钮会被禁用并显示说明文字。

**流程核验**需要手动开启。开启后，决策中心和复盘中心的交易核验会按你是否遵循了自己的流程给每笔决策打分（通过/不通过），而不只看交易结果。默认关闭（`config/verification.example.yaml` 中 `process_verification.enabled: false`）。要启用它，将示例配置复制到你的本地配置并设置为 true：

```bash
cp config/verification.example.yaml config/verification.yaml
```

然后编辑 `config/verification.yaml`，把 `process_verification.enabled` 改为 `true`，并重启应用（该设置在每个进程中只读取一次）。
