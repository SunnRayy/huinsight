...
✅ V3 Sync Complete
   ...
   Holdings:     229
   ...
   - Integrity gate: 14/16 passed. FAILED — 1 check(s) failed (0 blocking, 1 advisory): ADVISORY: ['unmatched_security_transfer']
```

Two of the 16 checks do not pass on a fresh demo, and both are expected:

- `unmatched_security_transfer` is an advisory finding on this dataset
  specifically — a known, harmless characteristic of the synthetic transfer
  history, not a bug in your setup.
- `twr_in_range` is **skipped**, not failed: it needs assets with a full
  year of history covering half the portfolio's value, and a fresh install's
  brokerage and fund readers are younger than that. It says so in its detail
  and starts checking on its own as history accumulates.

`--check-integrity --json` gives a machine-readable rerun without a full sync.

### Seed the demo AI content

The AI Advisor is the product's centerpiece, and its content (your investment
philosophy, strategy memos, briefs, reviews and insights) lives in the
database rather than in the source files above. Seed the demo persona's:

```bash
.venv/bin/python tools/demo_data/seed_ai.py
```

This writes an investor profile, the retirement goal the Forecast page
projects toward, three strategy memos, one sample Brief and
one sample Review (both labelled **Sample** in the app — written for the
persona, not generated by a model), insights at every lifecycle stage, and a
few verified trades. It only writes to a database that holds the demo
persona and refuses otherwise; `--dry-run` shows what it would write, and
...
The AI Advisor pages (profile, memos, insights, sample brief and review, context preview) work without any key. Generating new briefs, reviews, analyses and audits requires one LLM API key in the backend's environment. Set one of these variables before starting the app:
...
Then start the app normally: `./dev.sh start`. The app also reads a `.env` file in the repo root, so you can instead store keys there:
...
Without a key the Generate buttons on AI pages are disabled with an explanation.

**Process verification** is opt-in. When it is on, trade verification in the Decision Hub and Review Center scores each decision on whether you followed your process (a PASS/FAIL check), not only on how the trade turned out. It ships off by default (`process_verification.enabled: false` in `config/verification.example.yaml`). To turn it on, copy the example to your local config and set enabled to true:

```bash
cp config/verification.example.yaml config/verification.yaml
```

Then edit `config/verification.yaml`, change `process_verification.enabled` to `true`, and restart the app (the setting is read once per process).
