...
(`ux-command-center/src/context/useLanguage.tsx`). It covered the sidebar and nothing else.
...
`ux-command-center/src/i18n/index.ts` initializes i18next with statically-imported JSON catalogs (no HTTP
...
`normalizeLegacyStoredLanguage()` in `ux-command-center/src/i18n/index.ts` rewrites that value **once,
...
`ux-command-center/tests/i18n-zh-canary.test.tsx` flips to `zh-CN` and asserts real Chinese renders in the
...
| Skipping / deleting the 35 known-failing tests to get a green CI | Hides behaviour the owner has not yet ruled on (`ux-command-center/tests/operations-redesign.test.tsx`) and destroys the record of what is broken |
