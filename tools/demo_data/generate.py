...
def emit_ibkr(persona: dict[str, Any], out_dir: Path, rng: random.Random, ledger: Optional[dict] = None) -> list[Path]:
    ...
    if ledger is not None:
        # The newer report is the one the reader keeps; its value (positions +
        # cash) is what the pipeline will hold for IBKR at file prices.
        ledger["ibkr_usd"] = round(sum(p["qty"] * p["mark_price"] for p in positions2) + cash_usd, 2)
...
def emit_schwab_positions(persona: dict[str, Any], out_dir: Path, rng: random.Random, ledger: Optional[dict] = None) -> Path:
    ...
    if ledger is not None:
        ledger["schwab_usd"] = round(total_mkt_val, 2)
    lines.append(_schwab_row([
        "Positions Total", "", "--", "--", "--", "--",
...
def emit_cn_fund(persona: dict[str, Any], out_dir: Path, rng: random.Random, ledger: Optional[dict] = None) -> Path:
    ...
    # Base NAV per fund. These are real public fund codes, so the live price
    # refresh replaces the file NAV with the fund's real one; a random NAV
    # (the old 0.9-3.2 band) made the persona's value jump by ~¥1.2M the
    # moment prices refreshed, and fall back when they did not (Round 5 #2).
    # So each fund's NAV is anchored to its real recent NAV (persona `nav`),
    # and money-market funds stay at 1.0. The random draw is still consumed
    # so every later value in the dataset is unchanged by the anchoring.
    base_price: dict[str, float] = {}
    for f in catalog:
        drawn = rng.uniform(0.98, 1.02) if f["type"] == "货币型" else rng.uniform(0.9, 3.2)
        if f["type"] == "货币型":
            base_price[f["code"]] = 1.0
        else:
            base_price[f["code"]] = float(f["nav"]) if "nav" in f else round(drawn, 4)
    is_money = {f["code"]: f["type"] == "货币型" for f in catalog}
    ...
    for fund in catalog:
        ...
        for d in dates:
            ...
            if not is_money[code]:
                price_walk[code] = round(max(base_price[code] * 0.5, price_walk[code] * (1 + step)), 4)
    ...
    # Rescale each fund's price path so it ends exactly at its anchor NAV:
    # the history keeps its shape, and the snapshot NAV matches what a live
    # refresh would find (row layout: [date, code, name, type, amount, qty, price, fees, memo]).
    for code, anchor in base_price.items():
        final = price_walk[code]
        if not final:
            continue
        factor = anchor / final
        ...
        for r in txn_rows:
            if r[1] == code:
                r[6] = round(r[6] * factor, 4)
                r[4] = round(r[5] * r[6], 2)
        price_walk[code] = anchor
    ...
    if ledger is not None:
        # What the CN fund reader will hold at file NAVs (latest row per fund).
        ledger["cn_fund_cny"] = round(sum(r[6] for r in holdings_rows), 2)
...
def emit_financial_summary(
    ...
    # The dashboard's trend line is 合计总资产 and its latest point is the sum
    # of reader holdings, so the fund columns must end where the reader files
    # do — otherwise the last step of the chart is the gap between two
    # unrelated random series (Round 5 #2). Rescale each walk to end at the
    # file value; its shape (and the rng stream) is unchanged.
    if ledger is not None:
        if ledger.get("cn_fund_cny"):
            series = bs["投资资产_股票基金_A股基金"]
            k = ledger["cn_fund_cny"] / series[-1]
            bs["投资资产_股票基金_A股基金"] = [round(v * k, 2) for v in series]
        broker_usd = (ledger.get("schwab_usd") or 0.0) + (ledger.get("ibkr_usd") or 0.0)
        if broker_usd:
            k = broker_usd / usd_fund_usd[-1]
            usd_fund_usd = [round(v * k, 2) for v in usd_fund_usd]
    ...
    free_asset_cols = [
        ...
        "投资资产_银行理财_招行", "投资资产_长期保险_安泰人生",
    ]
    if ledger is None or not ledger.get("cn_fund_cny"):
        # Without a reconciled CN fund value the column stays free, as before.
        free_asset_cols.insert(4, "投资资产_股票基金_A股基金")
...
def generate_all(persona: dict[str, Any], out_dir: Path) -> list[Path]:
    ...
    # Values the reader files will hold, so the Financial Summary can reconcile to them.
    ledger: dict[str, float] = {}

    paths: list[Path] = []
    paths.extend(emit_ibkr(persona, out_dir, rng, ledger=ledger))
    paths.append(emit_schwab_positions(persona, out_dir, rng, ledger=ledger))
    ...
    paths.append(emit_cn_fund(persona, out_dir, rng, ledger=ledger))
