...
@pytest.fixture(autouse=True)
def _reset_market_refresh_guard():
    """Start every test with an empty market-refresh single-flight cache.

    src/market_data/refresh_guard.py keeps the last full refresh result
    process-wide for a cooldown window. Without a reset, a test that runs a
    refresh leaves that result behind, and a later test on the same xdist
    worker gets the "deduplicated recent result" shape instead of a fresh one
    — which failed public CI on 2026-09-25 purely by test ordering.
    """
