...
def test_get_decisions_stats_drift_basis_is_null_when_no_targets_exist(client):
    """Round 5 #8: a database with no Strategic_Profile targets and no active
    risk profile has no baseline to measure drift against at all. /decisions/stats
    must say so explicitly rather than let active_drift_alerts=0 imply a clean
    allocation. The `client` fixture applies schema + raw migrations but never
    runs the Python-side V193 risk-profile seed, so this is exactly that state."""
    response = client.get("/decisions/stats")
    assert response.status_code == 200
    data = response.json()
    assert data["active_drift_alerts"] == 0
    assert data["drift_basis"] is None
