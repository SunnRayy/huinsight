...
class TestProcessChecksEndpoint:
    def test_recent_trades_include_process_notes(self, process_client):
        client, db_path = process_client
        conn = DatabaseConnector(str(db_path))
        ...
        response = client.get("/ai-advisor/trades")

        assert response.status_code == 200
        trade = next(item for item in response.json()["trades"] if item["id"] == trade_id)
        assert trade["process_notes"] == "Transferred to IBKR; informational note only."
