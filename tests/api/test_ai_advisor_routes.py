...
def test_context_render_returns_full_review_prompt_draft(ai_advisor_client, monkeypatch):
    ...
    response = ai_advisor_client.post(
        "/ai-advisor/context/render",
        json={
            # Pinned: the preview now follows the resolved report language, and
            # this test asserts the English review scaffold.
            "language": "en",
            "report_type": "review",
            "context_config": {
                "tiers": {
...
def test_brief_generate_returns_503_when_llm_models_fail(ai_advisor_client, monkeypatch):
    ...
    # Rule 12 safe message: the upstream exception text is not echoed.
    assert "upstream 503" not in error["message"]
...
# ---------------------------------------------------------------------------
# No LLM key configured (Round 5 #4)
# ---------------------------------------------------------------------------

_GATED_REQUESTS = [
    ("/ai-advisor/brief/generate", {"context_config": {}}),
    ("/ai-advisor/review/questions", {"period_start": "2026-01-01", "period_end": "2026-01-31"}),
    ("/ai-advisor/review/generate", {
        "questions_answers": [], "period_start": "2026-01-01", "period_end": "2026-01-31", "context_config": {},
    }),
    ("/ai-advisor/review/cross-check", {"period_start": "2026-01-01", "period_end": "2026-01-31"}),
    ("/ai-advisor/analyze", {"asset_code": "US_STK_AAPL"}),
    ("/ai-advisor/memos/1/propose-updates", {}),
]


@pytest.mark.parametrize("path,body", _GATED_REQUESTS)
def test_generation_without_llm_key_explains_instead_of_failing(ai_advisor_client, monkeypatch, path, body):
    monkeypatch.setattr("src.api.routes.ai_advisor.llm_keys_configured", lambda: False)

    def _must_not_run(*_a, **_k):
        raise AssertionError("generation must not start without an LLM key")

    monkeypatch.setattr("src.services.ai_advisor.brief_generator.BriefGenerator.generate", _must_not_run)
    monkeypatch.setattr("src.services.ai_advisor.review_generator.ReviewGenerator.generate_questions", _must_not_run)
    monkeypatch.setattr("src.services.ai_advisor.review_generator.ReviewGenerator.generate_review", _must_not_run)
    monkeypatch.setattr("src.api.routes.ai_advisor.generate_cross_check_audit", _must_not_run)
    monkeypatch.setattr("src.api.routes.ai_advisor.propose_memo_updates", _must_not_run)

    response = ai_advisor_client.post(path, json=body)
...
def test_llm_status_reports_presence_only(ai_advisor_client, monkeypatch):
    monkeypatch.setattr("src.api.routes.ai_advisor.llm_keys_configured", lambda: False)
    assert ai_advisor_client.get("/ai-advisor/llm-status").json() == {"configured": False}
    monkeypatch.setattr("src.api.routes.ai_advisor.llm_keys_configured", lambda: True)
    assert ai_advisor_client.get("/ai-advisor/llm-status").json() == {"configured": True}
