...
def test_match_trades_to_insights_keeps_provenance_of_verified_rows_without_verdict():
    """A verified, verdict-less reader row keeps its 'imported' label.

    That provenance is what exempts it from integrity Rule B v2
    (trade_log_verdict_consistency). Relabeling it to 'memo' manufactured a
    violation the first time GET /decisions/scorecard ran on a DB with a memo
    mentioning the row's ticker (Round 5).
    """
    from src.services.decision_scorer import match_trades_to_insights
    ...
    conn.execute(
        ...
        VALUES ('2026-03-20', 'US_STK_VOO', 'Buy', 'imported', 'verified')
        """
    )
    conn.execute(
        """
        INSERT INTO strategy_memos (memo_date, title, strategic_bias, key_directives, source_file)
        VALUES ('2026-03-09', 'US core sleeve', 'neutral', '["Add VOO on dips"]', 'memo.md')
        """
    )

    assert match_trades_to_insights(conn) == 0
    assert conn.execute("SELECT suggestion_source FROM trade_logs").fetchone()[0] == "imported"
