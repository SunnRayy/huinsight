...
# ---------------------------------------------------------------------------
# Test 4: No trades after ai_since → unmeasured (score=None)
# ---------------------------------------------------------------------------

def test_no_trades_in_ai_window_returns_fallback(db_path):
    """With a memo but no post-AI trades, total_trades=0 → unmeasured (None)."""
    conn = duckdb.connect(db_path)
...
# ---------------------------------------------------------------------------
# Test 6: No strategy memos at all → ai_active_since = None → unmeasured (None)
# ---------------------------------------------------------------------------

def test_no_memos_returns_no_ai_data_fallback(db_path):
    """When strategy_memos is empty, _ai_active_since returns None → unmeasured (None)."""
