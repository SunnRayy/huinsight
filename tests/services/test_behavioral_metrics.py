...
def test_compute_all_handles_method_failure_gracefully():
    ...
    fallback = results[0]
    assert fallback.score is None  # unmeasured, never a plotted placeholder
    assert fallback.raw_value is None
...
def test_position_sizing_discipline_handles_missing_target_allocations():
    """If target_allocations table doesn't exist, return an unmeasured (None) score gracefully."""
    computer = _make_computer()
    ...
    assert "Could not compute drift" in result.description
...
def test_strategy_compliance_no_ai_data_fallback():
    """When _ai_active_since returns None, strategy_compliance is unmeasured (None) with a clear label."""
    computer = _make_computer()
