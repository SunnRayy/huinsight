...
def test_anthropic_models_omit_temperature(client):
    """Current Claude models 400 on a non-default temperature; it must not be sent."""
    assert client._sampling_kwargs("anthropic/claude-sonnet-5") == {}
    assert client._sampling_kwargs("gemini/gemini-2.5-flash") == {"temperature": 0.7}


def test_anthropic_models_get_thinking_headroom(client):
    """Adaptive thinking counts toward max_tokens, so Anthropic models get >= 16000."""
    assert client._max_tokens_for("anthropic/claude-sonnet-5") == 16000
    assert client._max_tokens_for("deepseek/deepseek-chat") == 4096


def test_estimate_cost_current_claude_model(client):
    """The Claude fallback slot is priced, not silently logged as $0."""
    cost = client._estimate_cost("anthropic/claude-sonnet-4-6", prompt_tokens=1000, completion_tokens=1000)
