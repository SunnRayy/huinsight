...
@patch("src.services.ai_advisor.review_generator.LLMClient")
def test_propose_memo_updates_accepts_fenced_json(MockLLM, tmp_path):
    """A reply wrapped in ```json fences must still yield proposals.

    LLMClient(expect_json=True) already strips fences and repairs the JSON into
    content_json; parsing the raw content instead silently returned zero proposals.
    """
    from src.services.ai_advisor.review_generator import propose_memo_updates

    conn = _make_db(tmp_path, "proposal_fenced.duckdb")
    _seed_memo(conn, 1)
    _seed_audit_report(conn)
    ...
    resp = _make_llm_response(diffs)
    resp.content = "```json\n" + json.dumps(diffs) + "\n```"
    resp.content_json = diffs
    mock_llm = MagicMock()
    MockLLM.return_value = mock_llm
