...
Written for Claude Code. Codex is opt-in only (the human asks for it explicitly).
...
| Source | Raw Currency | How `market_value` becomes CNY |
|--------|-------------|----------------------|
| PIS | CNY | Already CNY (qty x price x FX rate) |
| Schwab_CSV · Broker_IBKR · RSU_Excel | USD | `holdings.currency='USD'` (set by `_default_currency()` in `src/sync/phases/_common.py`); the price-refresh step (`src/sync/dsa_sync.py`) multiplies by the USD→CNY rate |
| Consolidated | USD | Sum of the co-authority broker rows it supersedes (Rule 4), already in CNY |
| CN_Fund_Excel · Gold_Excel · Insurance_Excel · Financial_Summary_Excel | CNY | Already CNY |

**A new USD source must be added to `_default_currency()`.** Otherwise `currency` defaults to CNY,
FX is applied as ×1.0, and `market_value` silently stays in raw USD (this is how IBKR shipped
unconverted). The fallback rate lives in one place — `get_default_usd_cny_rate()` in
`src/data_manager/currency_converter.py`, read from `currency.fallback_rates.USD_CNY` in
`config/settings.yaml` — never define a local `USD_TO_CNY` constant (verify.sh check).

**A USD-source position whose `market_value` is close to qty × USD price (rather than × FX)
was stored as raw USD** — investigate before concluding it's a bug. A single Schwab holding
worth ~$33K should appear as ~¥231K in the DB if correctly converted. Integrity check
`no_raw_usd_in_schwab_holdings` catches only the grossest case.
...
Reader data is **AUTHORITATIVE** over the PIS baseline (ADR-003), and the direction is one-way:
a reader row is never shadowed in favor of PIS. `is_shadow` is a *current-state* flag, and
three kinds of shadowed rows are legitimate:

- **PIS / PIS_Historical rows** covered by a reader source for the same asset.
- **Superseded reader snapshots** — older reader rows are shadowed when newer data lands. They
  are still the correct valuation *for their own date*, so point-in-time and historical queries
  must not filter `is_shadow = FALSE` (see `_latest_snapshot_by_asset` in
  `src/services/attribution.py`).
- **Co-authority broker rows** (`Schwab_CSV`, `Broker_IBKR`) superseded by an active
  `Consolidated` row (ADR-016, `_consolidate_coauthority_holdings`), and the zero-qty
  co-authority tombstones written by `_shadow_coauthority_tombstone` (both in
  `src/sync/phases/_shadow.py`).

**The bug** is a qty-bearing reader row at its source's latest snapshot date that is shadowed
with no active `Consolidated` row for the asset — reader data made invisible. Blocking integrity
checks `shadow_mutual_exclusion` (row level) and `reader_rows_not_all_shadowed` (whole source)
guard it. The reader list is `READER_SOURCES` in `data_integrity_gate.py` (from the reader
registry). Quick check, expecting zero rows:
```sql
WITH latest_source AS (
    SELECT source_system, MAX(snapshot_date) AS latest_date
    FROM holdings WHERE COALESCE(quantity, 0) != 0
    GROUP BY source_system
)
SELECT h.source_system, h.asset_id, h.snapshot_date
FROM holdings h
JOIN latest_source l ON h.source_system = l.source_system AND h.snapshot_date = l.latest_date
WHERE h.source_system IN ('Schwab_CSV','Broker_IBKR','CN_Fund_Excel','Gold_Excel','Insurance_Excel','RSU_Excel')
  AND h.is_shadow = TRUE
  AND COALESCE(h.quantity, 0) != 0
  AND NOT EXISTS (SELECT 1 FROM holdings c WHERE c.asset_id = h.asset_id
                  AND c.source_system = 'Consolidated' AND c.is_shadow = FALSE);
```
...
-- 2. Active vs shadowed by source (every reader source needs active rows;
--    shadowed historical reader rows are normal — Rule 4)
...
| `consolidated_equals_sum` | A `Consolidated` row ≠ sum of its broker rows | Broker row missed or double-counted in `_consolidate_coauthority_holdings`; check `AuthorityResolver.coauthority_sources()` |
| `unmatched_security_transfer` | Transfer leg with no same-asset counterpart within 7 days | Missing counterpart import (broker report gap) — confirm intent with the human |

Checks in `BLOCKING_CHECKS` (`src/validation/data_integrity_gate.py`) fail the sync
(`success=False`); the rest are advisory and set `degraded=True`.
...
| Duplicate market refreshes | Sync, the scheduler and the UI each trigger a full price refresh; concurrent runs repeat the quote fetches and tie up the single Cloud Run instance | Go through `run_guarded_refresh()` in `src/market_data/refresh_guard.py` (single-flight lock + cooldown); handle `RefreshInProgress` |
...
**Good — structured error envelope (HTTP 5xx + `{"error": {...}}`):**
```python
from src.api.routes._errors import api_error_response

except Exception as e:
    logger.exception("holdings endpoint failed")
    return api_error_response(e, context="holdings")
```

**Partial results — when independent sources feed one response, isolate them so one failure
doesn't blank the rest, and log each failure (the response must still let the frontend tell
...
The canonical example of this class: `^TNX` returned percent (4.455) and was divided by 10, producing 0.445%. Review new feed integrations against this rule before merge.

**All external HTTP calls must go through `http_get` from `src/utils/http_client.py`** — not bare `requests.get()`. The shared client provides retry logic, timeout enforcement, and a browser UA.
...
A real write→read→verify GCS round-trip (to confirm persistence works end-to-end) is appropriate as: (a) a deploy-time smoke test step in CI, or (b) a manual one-off command (none exists yet — `main.py` has no GCS round-trip flag). It must never be on the hot path of a health or status endpoint.
...
USD_TO_CNY_RATE = 7.0
# RIGHT
from src.data_manager.currency_converter import get_currency_service, get_default_usd_cny_rate
usd_to_cny = get_currency_service().get_latest_rate("USD", "CNY") or get_default_usd_cny_rate()
```
...
- Mis-shadowed reader rows at a source's latest snapshot — requires live DB query (AGENTS.md Rule 4)
