...
import { formatDate, formatMoneyStr, formatPercent, type UiLocale } from '../../src/utils/formatMoney';
...
export const HeroKpis: React.FC<HeroProps> = ({
  ...
  // Rule 22: say when the headline rests on non-live prices.
  const freshness = kpi?.price_freshness ?? null;
  const pricesDegraded = freshness?.status === 'unpriced' || freshness?.status === 'stale';
  const pricesNote = !freshness ? null
    : freshness.status === 'unpriced'
      ? t('dashboard.hero.pricesUnpriced', { pct: Math.round(freshness.unpriced_share * 100) })
      : freshness.status === 'stale' && freshness.last_price_at
        ? t('dashboard.hero.pricesStale', { date: formatDate(freshness.last_price_at, lang, { month: 'short', day: 'numeric', year: 'numeric' }) })
        : null;
  ...
  return (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 16 }}>
      ...
      <div style={cardStyle}>
        ...
        {pricesDegraded && pricesNote && (
          <div
            role="status"
            data-testid="net-worth-prices-degraded"
            style={{
              display: 'flex', alignItems: 'flex-start', gap: 6, marginTop: 10, padding: '6px 8px',
              borderRadius: 6, fontSize: 11, lineHeight: 1.35,
              background: freshness?.status === 'unpriced' ? 'var(--zone-orange-bg)' : 'var(--fill-neutral-soft)',
              color: freshness?.status === 'unpriced' ? 'var(--zone-orange-fg)' : 'var(--color-fg-3)',
            }}
          >
            <span className="material-symbols-outlined" style={{ fontSize: 14 }}>sync_problem</span>
            <span>{pricesNote}</span>
          </div>
        )}
      </div>
      ...
      {/* Vs Last Month */}
      <div style={cardStyle}>
        {/* Name the real baseline: the previous history point can be months old. */}
        <span style={eyebrowStyle}>{sinceLabel && sinceLabel !== '—' ? t('dashboard.hero.vsSince', { date: sinceLabel }) : t('dashboard.hero.vsLastMonth')}</span>
        {/* With non-live prices the change is not a market move — no red/green verdict. */}
        <div className="money-value" style={{ ...monoKpi, marginTop: 8, color: delta30d == null || freshness?.status === 'unpriced' ? 'var(--color-fg-4)' : (delta30d >= 0 ? 'var(--color-success)' : 'var(--color-danger)') }}>
          {delta30d != null ? fmt(delta30d, { signed: true }) : t('dashboard.hero.notAvailable')}
        </div>
        <div style={{ display: 'flex', gap: 14, marginTop: 8 }}>
          <div>
            <div style={subLabel}>{t('dashboard.hero.change')}</div>
            <div style={subVal(pct30d != null && freshness?.status !== 'unpriced' ? (pct30d >= 0 ? 'var(--color-success)' : 'var(--color-danger)') : 'var(--color-fg-4)')}>
