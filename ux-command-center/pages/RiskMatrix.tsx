...
export const RiskMatrix: React.FC = () => {
  ...
  const gridFrequency = correlation?.grid_frequency ?? '';
  const gridLabel = gridFrequency
    ? t(`riskMatrix.grid.${gridFrequency === 'W-FRI' ? 'weekly' : gridFrequency === 'ME' ? 'monthly' : 'daily'}`)
    : '';
  const windowDays = correlation?.window_days ?? 0;

  // window_end is the newest snapshot across ALL classes, which on a mixed-cadence
  // portfolio is always a daily class's date. Without this, a class whose newest
  // real observation is months old reads as current as the broker feed.
  const staleClasses = React.useMemo(() => {
    const evidence = correlation?.class_evidence;
    const end = correlation?.window_end;
    if (!evidence || !end) return [] as { name: string; lastObserved: string; daysStale: number }[];
    const endMs = Date.parse(end);
    if (Number.isNaN(endMs)) return [];
    return Object.entries(evidence)
      .map(([name, ev]) => {
        if (!ev?.last_observed) return null;
        const seenMs = Date.parse(ev.last_observed);
        if (Number.isNaN(seenMs)) return null;
        const daysStale = Math.round((endMs - seenMs) / 86_400_000);
        return daysStale > 30 ? { name, lastObserved: ev.last_observed, daysStale } : null;
      })
      .filter((x): x is { name: string; lastObserved: string; daysStale: number } => x !== null)
            ...
            <div>
                  ...
                  {correlation.matrix.map(row => (
                    <React.Fragment key={row.asset}>
                      ...
                      {correlation.assets.map(colAsset => {
                        ...
                        const bridgedPct = Math.round((cell?.bridged_share ?? 0) * 100);
                        const cellTitle = cell
                          ? t('riskMatrix.cellDetail', {
                              overlap: cell.overlap,
                              gridLabel: gridLabel || t('riskMatrix.grid.daily'),
                              bridged: bridgedPct,
                            })
                          : undefined;
                        return (
              ...
              {staleClasses.length > 0 && (
                <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-300">
                  {t('riskMatrix.staleClassNotice', {
                    classes: staleClasses
                      .map(c => t('riskMatrix.staleClassEntry', { name: c.name, date: c.lastObserved }))
                      .join('; '),
                  })}
                </div>
              )}
              <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-border-dark dark:bg-slate-900/40 dark:text-slate-300">
                ...
                {gridLabel && (
                  <span className="block mt-1">
                    {t('riskMatrix.gridSummary', {
                      gridLabel,
                      periods: correlation.effective_periods ?? 0,
                      windowDays,
                      bridged: Math.round((correlation.bridged_return_share ?? 0) * 100),
                      suppressed: correlation.fabricated_returns_suppressed ?? 0,
                    })}
                  </span>
                )}
              </div>
