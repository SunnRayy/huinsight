...
import { LlmKeyNotice } from '../components/ai-advisor/LlmKeyNotice';
import { useLlmStatus } from '../components/ai-advisor/useLlmStatus';
...
type ActiveTab = 'brief' | 'memos' | 'analyze' | 'record-trade' | 'review' | 'insights' | 'profile';
...
// where a new user would never find it. `isPhilosophyEmpty` decides whether
// the first-run callout below should point people at the new "Your profile"
// tab (same five fields InvestorProfileEditor edits).
function isPhilosophyEmpty(phil: InvestorPhilosophy | null): boolean {
  if (!phil) return true;
  return (
    !phil.goal?.trim() &&
    !phil.horizon?.trim() &&
    !phil.risk_tolerance?.trim() &&
    !phil.core_weakness?.trim() &&
    !phil.portfolio_structure?.trim()
  );
}
...
export const AIAdvisor: React.FC = () => {
  ...
  const llmOff = useLlmStatus() === false;
  ...
  const [philosophy, setPhilosophy] = useState<InvestorPhilosophy | null>(null);
  const [philosophyChecked, setPhilosophyChecked] = useState(false);
  ...
  // Separate, self-contained effect (own try/catch, not folded into the
  // Promise.allSettled above) so the profile-emptiness check never disturbs
  // brief/settings/metrics/insights loading if SettingsAPI is unavailable.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const profile = await SettingsAPI.getProfile();
        if (!cancelled) setPhilosophy(profile.philosophy ?? null);
      } catch {
        // non-critical — leave the first-run callout hidden on error
      } finally {
        if (!cancelled) setPhilosophyChecked(true);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleProfileSaved = useCallback((updated: InvestorPhilosophy) => {
    setPhilosophy(updated);
    setPhilosophyChecked(true);
  }, []);

  const showProfileCallout = philosophyChecked && isPhilosophyEmpty(philosophy) && activeTab !== 'profile';
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto w-full bg-gray-50 dark:bg-background-dark min-h-screen">
          ...
          {activeTab === 'brief' && (
            <>
              ...
              <button
                ...
                disabled={generating || !isPreviewCurrent || llmOff}
                title={llmOff ? t('llmKey.disabledTooltip') : undefined}
      ...
      {showProfileCallout && (
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl border border-primary/30 bg-primary/5 dark:bg-primary/10 px-4 py-3">
          <div className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-200">
            <span className="material-symbols-outlined !text-[18px] text-primary mt-0.5">person</span>
            <span>{t('profileCallout.message')}</span>
          </div>
          <button
            type="button"
            onClick={() => setActiveTab('profile')}
            className="flex-shrink-0 rounded-lg bg-primary px-3.5 py-1.5 text-xs font-semibold text-white hover:bg-primary/90 transition-colors self-start sm:self-auto"
          >
            {t('profileCallout.cta')}
          </button>
        </div>
      )}
      ...
      <div className="flex gap-1 mb-5 border-b border-slate-200 dark:border-border-dark overflow-x-auto">
        {(['brief', 'memos', 'analyze', 'record-trade', 'review', 'insights', 'profile'] as const).map((tab) => (
          <button
            key={tab}
            type="button"
          ...
          >
            {tab === 'brief' ? t('page.tabs.brief') :
             ...
             tab === 'review' ? t('page.tabs.review') :
             tab === 'insights' ? t('page.tabs.insights') : t('page.tabs.profile')}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'profile' ? (
        <div className="max-w-3xl">
          <InvestorProfileEditor onSaved={handleProfileSaved} />
        </div>
      ) : activeTab === 'insights' ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    {brief.model_used === 'demo-sample' && (
                      <p className="text-xs text-slate-400 mt-0.5">{t('demoSample.briefExplanation')}</p>
                    )}
                  </div>
                  <button
                    type="button"
