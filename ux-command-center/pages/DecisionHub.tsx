...
export const DecisionHub: React.FC = () => {
    ...
    // `drift_basis` is only ever explicitly null when neither the strategic nor
    // risk-profile scope has any targets to compare against — in that case
    // `driftAlertsCount` is 0 because drift is unmeasurable, not because
    // nothing has drifted, and the card must not claim otherwise (Round 5 #8).
    // `undefined` (a stats payload that predates this field) falls back to the
    // plain count, same as the other optional stats above.
    const driftBasis = stats?.drift_basis;
    const driftAlertsValue = driftBasis === null ? t('decisionHub.stats.driftNoTargets') : driftAlertsCount;
    const driftAlertsCaption = driftBasis === 'risk_profile'
        ? t('decisionHub.stats.driftBasisRiskProfile')
        : driftBasis === 'strategic'
            ? t('decisionHub.stats.driftBasisStrategic')
            : undefined;
...
const StatCard = ({ title, value, icon, color = 'text-slate-900 dark:text-white', onClick, caption }: any) => (
    <button
        type="button"
        onClick={onClick}
    ...
    >
        ...
        <div>
            ...
            {caption && <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{caption}</p>}
        </div>
    </button>
);
