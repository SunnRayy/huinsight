...
export interface KPI {
    ...
    /** How much of net_worth rests on live prices (null if it could not be computed). */
    price_freshness?: PriceFreshness | null;
}

export interface PriceFreshness {
    /** fresh | stale (newest live price is old) | unpriced (refresh failed) | none */
    status: 'fresh' | 'stale' | 'unpriced' | 'none';
...
export interface CorrelationCell {
    ...
    /** Share of this pair's return periods whose earlier endpoint was a carried
     *  value rather than a second observation. */
    bridged_share?: number;
}

/** Per-class cadence and recency. window_end is the newest snapshot across ALL
 *  classes, so a slow class can be months stale while the footer reads current. */
export interface CorrelationClassEvidence {
    observations: number;
...
export interface RiskCorrelation {
    ...
    /** Pandas frequency of the common period grid the returns were measured on
     *  ('D' | 'W-FRI' | 'ME'), chosen from the slowest class's own cadence. */
    grid_frequency?: string;
    grid_label?: string;
    /** Effective lookback. Scales with the grid: a monthly grid cannot produce
     *  8 return periods out of 180 days. */
    window_days?: number;
    bridged_return_share?: number;
    fabricated_returns_suppressed?: number;
    max_intra_period_lag_days?: number;
    mean_intra_period_lag_days?: number;
    class_evidence?: Record<string, CorrelationClassEvidence>;
}
...
export interface DecisionStats {
    ...
    /** Which allocation-target scope `active_drift_alerts` was computed against:
     *  'strategic' (target_allocations, the owner's hand-curated DB), 'risk_profile'
     *  (the active risk profile — every fresh install after migration V193), or
     *  null when neither scope has any targets, meaning drift is not measurable —
     *  `active_drift_alerts: 0` in that case is a missing baseline, not a clean
     *  bill of health. Absent entirely on a stats payload that predates this field. */
    drift_basis?: 'strategic' | 'risk_profile' | null;
...
export interface ProcessChecksResponse {
    id: number;
    process_notes: string | null;
    process_authorized?: boolean | null;
    process_params_ok?: boolean | null;
    process_data_verified?: boolean | null;
    process_checked_at?: string | null;
}

export interface UpdateProcessChecksRequest {
...
export interface BehavioralMetric {
  dimension: string;
  score: number | null;      // 0.0-1.0; null = not measured (never a placeholder number)
  raw_value: number | null;
  label: string;
  description: string;
  window_days?: number;
