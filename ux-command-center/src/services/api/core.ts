...
import type {
    ...
    ProcessChecksResponse, UpdateProcessChecksRequest,
...
export const coreApi = {
    ...
    updateProcessChecks: async (
        id: number,
        body: UpdateProcessChecksRequest,
    ): Promise<ProcessChecksResponse> => {
        const res = await authFetch(`${API_BASE}/ai-advisor/trades/${id}/process-checks`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
            const errorData = await res.json().catch(() => null);
            throw new Error(errorData?.detail || i18n.t('errors:core.updateProcessChecks'));
        }
        return res.json();
    },
