...
/** Whether any LLM provider key is configured on the backend (presence only). */
export const getLlmStatus = (): Promise<{ configured: boolean }> =>
  authFetch('/api/ai-advisor/llm-status').then(r => (r.ok ? r.json() : { configured: true }));
...
export const generateBrief = (
  ...
  }).then(async r => { if (!r.ok) throw new Error(await safeReadError(r, i18n.t('errors:aiAdvisor.briefGeneration', { status: r.status }))); return r.json(); });
...
export const renderAdvisorContext = (
  ...
  authFetch('/api/ai-advisor/context/render', {
    ...
    body: JSON.stringify({
      ...
      context_config: contextConfig,
      // Preview should match what an actual generate call would send the
      // LLM — same locale the other generate* calls in this file send.
      language: i18n.language,
      ...reviewPreview,
    }),
...
export const generateReviewQuestions = (
  ...
  }).then(async r => { if (!r.ok) throw new Error(await safeReadError(r, i18n.t('errors:aiAdvisor.questions', { status: r.status }))); return r.json(); });
...
export const generateReview = (
  ...
  }).then(async r => { if (!r.ok) throw new Error(await safeReadError(r, i18n.t('errors:aiAdvisor.review', { status: r.status }))); return r.json(); });
...
export async function analyzeAsset(assetCode: string, analysisType = 'full'): Promise<AnalysisResult> {
  ...
  if (!res.ok) throw new Error(await safeReadError(res, i18n.t('errors:aiAdvisor.analysis', { status: res.status })));
  return res.json();
}
