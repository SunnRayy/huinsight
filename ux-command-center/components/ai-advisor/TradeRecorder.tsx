...
export const TradeRecorder: React.FC = () => {
  ...
  const [noteTrade, setNoteTrade] = useState<TradeLogEntry | null>(null);
  ...
  const handleOpenNote = (e: React.MouseEvent, trade: TradeLogEntry) => {
    e.stopPropagation();
    setError(null);
    setNoteTrade(trade);
    setNoteDraft(trade.process_notes || '');
  };

  const handleSaveNote = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!noteTrade || !noteDraft.trim()) return;

    setSavingNote(true);
    setError(null);
    try {
      const response = await api.updateProcessChecks(noteTrade.id, { notes: noteDraft.trim() });
      const processNotes = response.process_notes ?? noteDraft.trim();
      setTrades((current) => current.map((trade) => (
        trade.id === noteTrade.id ? { ...trade, process_notes: processNotes } : trade
      )));
      setNoteTrade(null);
      setNoteDraft('');
    } catch (e) {
      console.error('Process note save failed', e);
      setError(e instanceof Error ? e.message : t('tradeRecorder.errors.saveProcessNote'));
    } finally {
      setSavingNote(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  ...
                  <th className="px-3 py-2 text-[10px] font-semibold uppercase text-slate-500">{t('tradeRecorder.note')}</th>
                ...
                {trades.map((trade) => {
                  ...
                  return (
                    <tr key={trade.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="px-3 py-2.5 text-xs text-slate-600 dark:text-slate-400">
                        {trade.process_notes ? (
                          <span className="inline-block max-w-[220px] truncate" title={trade.process_notes}>
                            {trade.process_notes}
                          </span>
                        ) : (
                          '—'
                      ...
                      <td className="px-3 py-2.5 text-right">
                        <button
                          type="button"
                          onClick={(e) => handleOpenNote(e, trade)}
                          className="mr-2 transition-colors hover:text-primary"
                          title={t('tradeRecorder.addProcessNote')}
                        >
      ...
      {noteTrade && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-slate-950/40 px-4" role="dialog" aria-modal="true">
          <form onSubmit={handleSaveNote} className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-xl dark:border-border-dark dark:bg-card-dark">
            <h3 className="mb-1 text-sm font-semibold text-slate-800 dark:text-slate-100">{t('tradeRecorder.processNoteTitle')}</h3>
            <p className="mb-4 text-xs text-slate-500">{noteTrade.asset_name || noteTrade.asset_id} · {noteTrade.log_date}</p>
            <label htmlFor="process-note" className="mb-1 block text-xs text-slate-500">{t('tradeRecorder.processNoteLabel')}</label>
            <textarea
              id="process-note"
              aria-label={t('tradeRecorder.processNoteLabel')}
              rows={4}
              value={noteDraft}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setNoteTrade(null)} className="rounded-md border border-slate-200 px-4 py-2 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-300">
                {t('tradeRecorder.cancelNote')}
              </button>
              <button type="submit" disabled={savingNote || !noteDraft.trim()} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
                {savingNote ? t('tradeRecorder.savingNote') : t('tradeRecorder.saveNote')}
              </button>
            </div>
          </form>
        </div>
