...
import { resetLlmStatusCache } from './useLlmStatus';
...
beforeEach(() => {
  ...
  vi.mocked(api.getLlmStatus).mockResolvedValue({ configured: true });
});
...
describe('AssetAnalyzer', () => {
  ...
  it('disables Analyze with an explanation when no LLM key is configured', async () => {
    vi.mocked(api.getLlmStatus).mockResolvedValue({ configured: false });
    vi.mocked(api.searchAnalyzableAssets).mockResolvedValue(mockSearchResults);
    render(<AssetAnalyzer />);

    fireEvent.change(screen.getByPlaceholderText(/Search asset by code or name/i), { target: { value: 'AAPL' } });
    await waitFor(() => expect(screen.getByText('Apple Inc.')).toBeInTheDocument());
    fireEvent.click(screen.getByText('Apple Inc.'));

    const analyzeButton = screen.getByRole('button', { name: /Analyze/i });
    await waitFor(() => expect(analyzeButton).toBeDisabled());
    expect(analyzeButton).toHaveAttribute('title', 'Add an LLM API key to enable generation');
