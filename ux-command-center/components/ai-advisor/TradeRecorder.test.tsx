...
describe('TradeRecorder', () => {
  ...
  it('allows adding a process note to an imported trade without deleting it', async () => {
    vi.mocked(api.listTrades).mockResolvedValueOnce({ trades: mockTrades });
    vi.mocked(api.updateProcessChecks).mockResolvedValueOnce({
      id: 2,
      process_notes: 'Transferred to IBKR; informational note only.',
    });
    ...
    fireEvent.click(screen.getAllByTitle('Add process note')[1]);
    fireEvent.change(screen.getByLabelText('Process note'), {
      target: { value: 'Transferred to IBKR; informational note only.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Save note' }));

    await waitFor(() => {
      expect(api.updateProcessChecks).toHaveBeenCalledWith(2, {
        notes: 'Transferred to IBKR; informational note only.',
      });
    });
    expect(await screen.findByText('Transferred to IBKR; informational note only.')).toBeInTheDocument();
  });
