...
function dimensionLabels(t: TFunction<'aiAdvisor'>): Record<string, string> {
  return {
    ...
    // metric — deliberately unscored (null) by the backend so radar geometry
...
export const BehavioralRadar: React.FC<BehavioralRadarProps> = ({ metrics, onCompute, computing }) => {
  ...
  // Only measured dimensions go on the shape. A null score means "not
  // measured"; plotting it at any fixed value would present a guess as a
  // measurement, so it is left off rather than drawn at 0 or mid-scale.
  const scored = metrics.filter((m): m is BehavioralMetric & { score: number } => m.score != null);
  const radarData = scored.map((m) => ({
    subject: DIMENSION_LABELS[m.dimension] || m.dimension,
    score: Math.round(m.score * 100),
    fullMark: 100,
  }));
  // A radar needs at least three axes to form a polygon.
  const canDrawShape = scored.length >= 3;
      ...
      {metrics.length === 0 ? (
        ...
        <>
          ...
          <div className="h-64 w-full mb-2">
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center" data-testid="radar-too-few">
              <span className="material-symbols-outlined !text-[32px] text-slate-300 dark:text-slate-600 mb-2">
                radar
              </span>
              <p className="text-xs text-slate-400 max-w-xs">
                {t('behavioralRadar.tooFewToDraw', { scored: scored.length, total: metrics.length })}
              </p>
            </div>
          )}
          {canDrawShape && scored.length < metrics.length && (
            <p className="text-[11px] text-slate-400 text-center mb-3" data-testid="radar-partial">
              {t('behavioralRadar.partialShape', { scored: scored.length, total: metrics.length })}
            </p>
          )}
          ...
          <div className="space-y-0.5">
            {metrics.map((m) => (
              <div key={m.dimension} className="flex items-center gap-3 py-1.5">
                ...
                {m.score == null ? (
                  <div
                    className="flex-1 h-1.5 rounded-full border border-dashed border-slate-300 dark:border-slate-600"
                    title={t('behavioralRadar.notMeasured')}
                    aria-label={t('behavioralRadar.notMeasured')}
                    data-testid={`metric-unmeasured-${m.dimension}`}
                  />
                ) : (
