...
from src.market_data.refresh_guard import RefreshInProgress, run_guarded_refresh
...
def _refresh_and_persist(writable: DatabaseConnector) -> dict:
...
def _run_refresh_request(db_path: str) -> dict:
    def refresh_and_persist() -> dict:
        writable = DatabaseConnector(db_path, read_only=False)
        try:
            return _refresh_and_persist(writable)
        finally:
            writable.close()

    return run_guarded_refresh(refresh_and_persist)


@router.post("/refresh")
async def refresh_market_data(db: DatabaseConnector = Depends(get_db)):
    # Full refresh performs network I/O for every active asset. Release the
    # read-only dependency connection before opening the worker's writer, then
    # keep the event loop responsive while quote providers run.
    db_path = db.db_path
    ...
    try:
        if db_path == ":memory:":
            return await asyncio.to_thread(
                lambda: run_guarded_refresh(lambda: _refresh_and_persist(db))
            )
        return await asyncio.to_thread(_run_refresh_request, db_path)
    except RefreshInProgress as exc:
        raise HTTPException(
            status_code=409,
            detail="market_data_refresh_in_progress",
