...
from src.services.llm_client import LLMAllModelsFailedError, llm_keys_configured
...
_LLM_NOT_CONFIGURED_MESSAGE = (
    "No LLM API key is configured. Set GEMINI_API_KEY, DEEPSEEK_API_KEY or "
    "ANTHROPIC_API_KEY for the backend process to enable AI generation."
)


def _llm_not_configured_response() -> Optional[JSONResponse]:
    """Rule 12 envelope for "no key configured", or None when a key is set.

    Checked before any context building or model call so a keyless install gets
    an explanation instead of a bare 503 after a pointless fallback walk.
    """
    if llm_keys_configured():
        return None
    return JSONResponse(
        status_code=503,
        content={"error": {"code": LLM_NOT_CONFIGURED_CODE, "message": _LLM_NOT_CONFIGURED_MESSAGE}},
    )


def _ai_advisor_error_response(exc: Exception) -> JSONResponse:
    """Rule 12 envelope for a failed generation. Never echoes str(exc)."""
    if isinstance(exc, LLMAllModelsFailedError):
        return JSONResponse(
            status_code=503,
            content={"error": {
                "code": "llm_unavailable",
                "message": "Every configured LLM model failed. Check the backend log and the provider status.",
            }},
        )
    return api_error_response(exc, context="ai_advisor")
...
class ContextRenderRequest(BaseModel):
    ...
    #: Per-request narrative/scaffold language override (the UI's current
    #: locale). None resolves via the same precedence a real generation call
    #: uses (see `_resolved_request_language`), so this preview matches what
    #: brief/review generation would actually send the LLM.
    language: Optional[str] = None
...
async def render_context_preview(request: ContextRenderRequest):
    ...
    try:
        ...
        lang = _resolved_request_language(request.language)
        cb = context_builder_module.ContextBuilder()
        context_builder_module.use_language(cb, lang)
        context_text = context_builder_module.render_context(cb, request.context_config, language=lang)
...
async def run_analysis(body: AnalyzeRequest):
    ...
    if (not_configured := _llm_not_configured_response()) is not None:
...
async def get_llm_status():
    """Whether any LLM provider key is configured — lets the UI disable Generate
    buttons up front instead of failing after a click. Reports presence only;
    never key values."""
    return {"configured": llm_keys_configured()}
...
def generate_brief(request: BriefGenerateRequest):
    ...
    try:
        ...
        return _ai_advisor_error_response(e)
...
async def list_trades(limit: int = Query(default=50, ge=1, le=1000)):
    ...
    try:
        ...
        try:
            ...
            select_process_notes = ", process_notes" if _table_has_column(conn, "trade_logs", "process_notes") else ", NULL AS process_notes"
            rows = conn.execute(
                f"""
                SELECT id, log_date, asset_id, asset_name, action, price, quantity,
                       amount, currency, decision_reason, suggestion_source{select_linked_memo}{select_verification_status}{select_process_notes}
                FROM trade_logs
