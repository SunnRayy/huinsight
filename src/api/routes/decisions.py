...
from src.services.alert_generator import generate_alerts, drift_basis
...
async def get_decisions_stats(db: DatabaseConnector = Depends(get_db)):
    ...
    stats = {
        ...
        # 'strategic' | 'risk_profile' | None — which target scope drift alerts
        # are (or would be) computed against. None means neither scope has any
        # targets, so `active_drift_alerts: 0` above is not measurable, not a
        # clean bill of health. See alert_generator.drift_basis().
        "drift_basis": drift_basis(db),
