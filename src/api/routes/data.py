...
# Floor on the correlation lookback, not a fixed span: a coarser grid needs a
# longer window to reach a usable number of periods (see _correlation_grid).
CORR_WINDOW_DAYS = 180
CORR_MIN_OVERLAP_PERIODS = 8
CORR_JUMP_K = 6.0
CORR_JUMP_ABS_FLOOR = 0.30       # calibrated for a 1-day period; scaled by grid
CORR_JUMP_ABS_HARD_CAP = 2.0     # calibrated for a 1-day period; scaled by grid
CORR_WINSOR_P_LOW = 0.05
CORR_WINSOR_P_HIGH = 0.95
CORR_LOW_CONFIDENCE_MARGIN = 3

# A cell whose returns are mostly bridged (numerator real, denominator carried)
# is measuring less than it appears to; mark it low-confidence in the matrix.
CORR_BRIDGED_LOW_CONFIDENCE_SHARE = 0.34
...
async def get_dashboard_kpi(
    ...
    try:
        ...
        # How much of that number rests on live prices (Rule 22 staleness).
        try:
            from src.services.price_freshness import compute_price_freshness
            price_freshness = compute_price_freshness(db)
        except Exception:
            logger.exception("dashboard/kpi: price freshness unavailable")
        ...
        # Never a fabricated net_worth=0 with 200: the headline card cannot tell
        # "portfolio is empty" from "query failed" (AGENTS.md Rule 12).
        logger.exception("dashboard/kpi failed")
        return api_error_response(e, context="dashboard_kpi")
...
async def get_risk_correlation(
    ...
    try:
        ...
        from src.api.routes._correlation_grid import (
            GRID_LABELS,
            build_grid,
            class_evidence as class_evidence_for,
            classify_returns,
            detect_grid_frequency,
            intra_period_lag_days,
            jump_scale_for,
            window_days_for,
        )
        ...
        def empty_payload(method: str = "empirical_holdings", window_start: str = "", window_end: str = ""):
            return {
                ...
                "bridged_return_share": 0.0,
                "fabricated_returns_suppressed": 0,
                "max_intra_period_lag_days": 0,
                "mean_intra_period_lag_days": 0.0,
                "class_evidence": {},
            }
        ...
        # Pick the grid from the data BEFORE trimming, so a slow class's cadence
        # is measured over its own history rather than over whatever survives a
        # 180-day cut (six month-end snapshots is not enough to time a cadence).
        grid_freq, _slowest_gap = detect_grid_frequency(pivot)
        window_days = window_days_for(grid_freq, CORR_WINDOW_DAYS)

        cutoff = pivot.index.max() - pd.Timedelta(days=window_days)
        ...
        # Align every class onto one common period grid before differencing.
        #
        # This replaces pct_change() on the raw union index, which shifted by one
        # union row and so gave a class a return only when it was stamped on two
        # ADJACENT union dates. With one daily class in the portfolio the union
        # index is daily, and every coarser class yielded zero returns — not too
        # few, zero — regardless of window length. See issue #34 and the module
        # docstring in _correlation_grid.py for the measurements.
        levels, real_mask, obs_dates = build_grid(pivot, grid_freq)
        returns, bridged_mask, fabricated_suppressed = classify_returns(levels, real_mask)
        max_lag_days, mean_lag_days = intra_period_lag_days(obs_dates)
        class_evidence = class_evidence_for(pivot)

        # Jump detection is calibrated for a 1-day period; rescale it to the grid
        # so an ordinary month-on-month cash swing is not flagged as a structural
        # break (that miscalibration was eating overlap from the thinnest series).
        jump_scale = jump_scale_for(grid_freq)
        jump_abs_floor = CORR_JUMP_ABS_FLOOR * jump_scale
        jump_abs_hard_cap = CORR_JUMP_ABS_HARD_CAP * jump_scale
        ...
        for asset in returns_masked.columns:
            ...
            jump_mask = ((series.abs() > threshold) | (series.abs() > jump_abs_hard_cap)) & (
                series.abs() > jump_abs_floor
            )
        ...
        for a1 in assets:
            ...
            for a2 in assets:
                ...
                if a1 == a2:
                    # A class with no usable returns has not been shown to
                    # correlate perfectly with itself — nothing was measured.
                    # Reporting 1.00 there is the same fabrication as any other
                    # number with no evidence behind it.
                    correlations[a2] = {
                        "value": 1.0 if overlap > 0 else None,
                        "overlap": overlap,
                    ...
                    # How much of this pair's evidence rests on a carried
                    # denominator rather than two observations.
                    pair_rows = returns_clean[[a1, a2]].dropna().index
                    if len(pair_rows):
                        bridged_hits = int(
                            (bridged_mask.loc[pair_rows, a1] | bridged_mask.loc[pair_rows, a2]).sum()
                        )
                        bridged_share = bridged_hits / len(pair_rows)
                    else:
                        bridged_share = 0.0
                    thin = (
                        CORR_MIN_OVERLAP_PERIODS
                        <= overlap
                        <= (CORR_MIN_OVERLAP_PERIODS + CORR_LOW_CONFIDENCE_MARGIN)
                    )
                    # Low confidence covers measured bias as well as thinness: a
                    # mostly-bridged cell looks as solid as a fully observed one
                    # unless the matrix itself says otherwise.
                    low_conf = corr_value is not None and (
                        thin or bridged_share >= CORR_BRIDGED_LOW_CONFIDENCE_SHARE
                    )
                    correlations[a2] = {
                        "value": corr_value,
                        "overlap": overlap,
                        "low_confidence": bool(low_conf),
                        "bridged_share": round(bridged_share, 4),
                    }
        ...
        total_emitted = int(returns.notna().to_numpy().sum())
        bridged_total = int(bridged_mask.to_numpy().sum())

        return {
            "matrix": matrix,
            "assets": assets,
            "method": "empirical_holdings",
            # Periods on the grid the returns were actually measured on, not rows
            # of the raw union index — those two numbers diverge by an order of
            # magnitude on a mixed-cadence portfolio.
            "effective_periods": max(len(levels) - 1, 0),
            ...
            "grid_label": GRID_LABELS.get(grid_freq, grid_freq),
            "window_days": window_days,
            "bridged_return_share": (
                round(bridged_total / total_emitted, 4) if total_emitted else 0.0
            ),
            "fabricated_returns_suppressed": fabricated_suppressed,
            "max_intra_period_lag_days": max_lag_days,
            "mean_intra_period_lag_days": round(mean_lag_days, 2),
            "class_evidence": class_evidence,
        }
    except Exception as e:
        # Rule 12: an empty matrix with HTTP 200 is indistinguishable from "this
        # portfolio has no correlations", which is exactly the silent-failure
        # shape commit c085931 removed from /risk/metrics. The client throws and
        # the page renders its error state instead.
        logger.exception("risk correlation failed")
        return api_error_response(e, context="risk_correlation")
