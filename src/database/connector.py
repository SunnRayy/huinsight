...
# Labels that behavioral_metrics wrote only on its placeholder (score=0.5)
# paths before 2026-09-24. Used by migration V194 to identify those rows.
_BEHAVIORAL_PLACEHOLDER_LABELS = (
    "Insufficient transaction data",
    "No data",
    "No price data",
    "No target allocation data",
    "N/A",
    "No AI advisory data yet",
    "No trades in AI-active window",
)
_BEHAVIORAL_PLACEHOLDER_LABELS_SQL = ", ".join(f"'{label}'" for label in _BEHAVIORAL_PLACEHOLDER_LABELS)
...
class DatabaseConnector:
    ...
    def run_migrations(self) -> None:
        ...
        # ── V194: un-fabricate stored behavioral placeholder scores ───────────
        # Until 2026-09-24 every insufficient-data path in behavioral_metrics
        # stored score=0.5 (raw 0.0 or 50.0), and manual_contrarian stored a
        # pinned 0.5 even with data. The radar reads the latest row per
        # dimension, so those placeholders kept plotting as real mid-scale
        # measurements until the next recompute. The labels below are written
        # only by those placeholder paths, so they identify the rows exactly.
        # manual_contrarian keeps a measured raw rate; only its score goes.
        self._apply_versioned_migration(
            194, "V194 null behavioral placeholder scores",
            f"""
            UPDATE ai_behavioral_log
            SET score = NULL,
                raw_value = CASE
                    WHEN json_extract_string(metadata_json, '$.label') IN ({_BEHAVIORAL_PLACEHOLDER_LABELS_SQL})
                    THEN NULL ELSE raw_value END
            WHERE dimension = 'manual_contrarian'
               OR json_extract_string(metadata_json, '$.label') IN ({_BEHAVIORAL_PLACEHOLDER_LABELS_SQL})
            """,
        )
