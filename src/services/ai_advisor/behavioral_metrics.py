...
class MetricResult:
    dimension: str
    # 0.0-1.0 normalized, or None when the dimension could not be measured
    # (insufficient data, or not a scored dimension). None is never a
    # placeholder number: a fabricated 0.5 plots as a real mid-scale value
    # (AGENTS.md Core Doctrine; guarded by tests/services/ai_advisor/
    # test_behavioral_no_placeholder_score.py).
    score: Optional[float]
    raw_value: Optional[float]      # raw computed value (%, days, etc.), None when unmeasured
...
class BehavioralMetricsComputer:
    ...
    def _manual_contrarian(self, window_days: int, conn: Optional[object] = None) -> MetricResult:
        ...
        Deliberately unscored (score=None) regardless of the raw rate: the owner's documented weakness is manual dip-buying
        ...
        the score/radar geometry. It was once pinned at a neutral 0.5, but a
        constant plotted on the radar reads as a mid-scale measurement, so it is
        now left out of the radar shape — only the alert and raw_value
        ...
        if result.label in ("No data", "No price data"):
            result.description = (
                ...
                "metric (manual dip-buying); never scored on the radar."
            )
            result.metadata["alert"] = False
            return result

        # Never scored — this metric must not reward/punish via radar geometry
        # (see docstring above); raw_value and metadata['alert'] carry the signal.
        result.score = None
        ...
        result.description = (
            ...
            "not scored on the radar (unlike systematic_contrarian, which reflects preset execution discipline "
    ...
    def _position_sizing_discipline(self, window_days: int, conn: Optional[object] = None) -> MetricResult:
        ...
        try:
            ...
            return MetricResult(
                ...
                computation_window_days=window_days,
                label="N/A",
                description="Could not compute drift from target allocation",
            )
    ...
    def _decision_speed(self, window_days: int, conn: Optional[object] = None) -> MetricResult:
        ...
        try:
            ...
            return MetricResult(
                ...
                computation_window_days=window_days,
                label="Insufficient transaction data",
                description="Not enough insight→trade history yet",
            )
    ...
    def _rebalance_discipline(self, window_days: int, conn: Optional[object] = None) -> MetricResult:
        ...
        try:
            ...
            # Targets exist but nothing is held: every class would read as
            # "drifted" from zero, which is not a measurement of discipline.
            if not any((r.get("current_value") or 0) > 0 for r in carved_rows):
                return MetricResult(
                    dimension="rebalance_discipline",
                    score=None,
                    raw_value=None,
                    computation_window_days=window_days,
                    label="No holdings to compare",
                    description="Target allocation exists but there are no current holdings to measure drift against",
                )
