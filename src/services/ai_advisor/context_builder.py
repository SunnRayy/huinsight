...
# ---------------------------------------------------------------------------
# Language handling for rendered context block headers/labels (Round 5 #6)
# ---------------------------------------------------------------------------
# Every block header and fixed label emitted into the LLM context text used to
# be a literal Chinese string with no language handling at all — so an
# English-UI "Preview Context" still showed `## 投资者画像` etc. This mirrors
# prompts.py's bilingual scaffold (a dict per label, "zh-CN"/"en" siblings in
# the same literal so an edit to one sits next to the other in the diff), but
# stays local to this module: it is a pure text-scaffolding concern, not the
# narrative output language contract prompts.py owns.
#
# Data VALUES (asset names, holdings, memo bodies, numbers) are NEVER
# translated here — only the fixed scaffolding around them.
#
# `_DEFAULT_CONTEXT_LANGUAGE` is deliberately "zh-CN", not
# `section_ids.DEFAULT_LANGUAGE` ("en"): this module predates language
# support, every existing caller and stored `context_config_json` assumed
# Chinese output, so `language=None` (or an unrecognised code) must keep
# producing byte-identical text — nothing already generated or under test
# silently changes.
_DEFAULT_CONTEXT_LANGUAGE = "zh-CN"

_LABELS: dict[str, dict[str, str]] = {
    "minutes_ago": {"zh-CN": "{n}分钟前", "en": "{n} minutes ago"},
    "hours_ago": {"zh-CN": "{n}小时前", "en": "{n} hours ago"},

    "investor_profile_header": {"zh-CN": "## 投资者画像", "en": "## Investor Profile"},
    "investor_label": {"zh-CN": "- 投资者：{name}", "en": "- Investor: {name}"},
    "goal_label": {"zh-CN": "- 目标：{value}", "en": "- Goal: {value}"},
    "risk_tolerance_label": {"zh-CN": "- 风险承受：{value}", "en": "- Risk Tolerance: {value}"},
    "horizon_label": {"zh-CN": "- 期限：{value}", "en": "- Horizon: {value}"},
    "core_weakness_label": {"zh-CN": "- 核心弱点：{value}", "en": "- Core Weakness: {value}"},
    "risk_profile_label": {"zh-CN": "- 风险画像：{value}", "en": "- Risk Profile: {value}"},
    "allocation_join_sep": {"zh-CN": "，", "en": ", "},
    "target_allocation_label": {"zh-CN": "- 目标配置：{value}", "en": "- Target Allocation: {value}"},
    "allocation_logic_label": {"zh-CN": "- 配置逻辑：{value}", "en": "- Allocation Logic: {value}"},

    "ai_insights_header": {"zh-CN": "## AI洞见沉淀", "en": "## AI Insights"},

    "portfolio_header": {"zh-CN": "## 当前资产配置", "en": "## Current Asset Allocation"},
    "no_holdings_data": {"zh-CN": "- 暂无持仓数据", "en": "- No holdings data available"},
    "portfolio_unavailable": {
        "zh-CN": "## 当前资产配置\n\n- 数据暂不可用",
        "en": "## Current Asset Allocation\n\n- Data temporarily unavailable",
    },
    "largest_drift_header": {"zh-CN": "\n### 偏离最大的配置", "en": "\n### Largest Allocation Drift"},
    "drift_line": {
        "zh-CN": "- {class_name}: 当前{current_pct:.1f}% vs 目标{target_pct:.1f}% (偏离{drift_pct:.1f}%)",
        "en": "- {class_name}: current {current_pct:.1f}% vs target {target_pct:.1f}% (drift {drift_pct:.1f}%)",
    },
    "holdings_detail_header": {"zh-CN": "\n### 持仓明细", "en": "\n### Holdings Detail"},
    "holdings_table_header": {
        "zh-CN": "| 资产 | 市值(CNY) | Lifetime P&L | Return % | 权重% |",
        "en": "| Asset | Market Value (CNY) | Lifetime P&L | Return % | Weight % |",
    },

    "performance_header": {"zh-CN": "## 绩效与风险指标", "en": "## Performance & Risk Metrics"},

    "market_header": {"zh-CN": "## 市场情绪", "en": "## Market Sentiment"},
    "raw_market_cache_header": {"zh-CN": "\n### 原始市场缓存", "en": "\n### Raw Market Cache"},

    "strategy_header": {"zh-CN": "## 策略备忘", "en": "## Strategy Memos"},
    "no_body": {"zh-CN": "(无正文)", "en": "(no content)"},

    "transactions_header": {"zh-CN": "## 近期交易", "en": "## Recent Transactions"},
    "tx_table_header_summary": {
        "zh-CN": "| 日期 | 资产 | 操作 | 数量 | 价格 | 评级 |",
        "en": "| Date | Asset | Action | Quantity | Price | Grade |",
    },
    "tx_table_header_detailed": {
        "zh-CN": "| 日期 | 资产 | 操作 | 数量 | 价格 | 金额 | 评级 |",
        "en": "| Date | Asset | Action | Quantity | Price | Amount | Grade |",
    },
    "tx_table_header_full": {
        "zh-CN": "| 日期 | 资产 | 操作 | 数量 | 价格 | 金额 | 评级 | 原因 | AI建议 | 验证 |",
        "en": "| Date | Asset | Action | Quantity | Price | Amount | Grade | Reason | AI Suggestion | Verification |",
    },

    "realtime_header": {"zh-CN": "## 实时市场指标", "en": "## Real-Time Market Indicators"},

    "valuation_header": {"zh-CN": "## 估值仪表盘\n", "en": "## Valuation Dashboard\n"},
    "valuation_note": {
        "zh-CN": "> ⚠️ 数据说明：以下估值数据来自系统快照，**仅供参考，非实时数据**。请务必结合最新市场信息交叉验证后再做决策。",
        "en": (
            "> ⚠️ Data note: the valuation data below comes from a system "
            "snapshot, **for reference only, not real-time**. Always cross-check "
            "against current market information before deciding."
        ),
    },
    "valuation_table_header": {
        "zh-CN": "| 资产 | 类型 | 当前估值 | 历史%位 | 信号 | 更新日期 |",
        "en": "| Asset | Type | Current Valuation | Historical %ile | Signal | Updated |",
    },

    "technical_score": {"zh-CN": "评分{value}/100", "en": "Score {value}/100"},
    "technical_volume": {"zh-CN": "量能={value}", "en": "Volume={value}"},
    "technical_support": {"zh-CN": "支撑={value}", "en": "Support={value}"},
    "technical_resistance": {"zh-CN": "阻力={value}", "en": "Resistance={value}"},
    "technical_header": {"zh-CN": "## 近期技术分析\n", "en": "## Recent Technical Analysis\n"},

    "context_unavailable_fallback": {
        "zh-CN": "（持仓和市场数据暂不可用。请仅说明数据不足，不要编造分析。）",
        "en": (
            "(Holdings and market data are currently unavailable. State that data "
            "is insufficient rather than fabricating analysis.)"
        ),
    },
}


def _resolve_context_language(language: Optional[str]) -> str:
    """Normalize to a supported context-scaffolding language.

    Unlike ``language_resolver.resolve_language`` (which reads the
    request/user_profile/settings precedence for the LLM's own narrative
    output language), this is a pure fallback for the FIXED scaffolding this
    module renders: an unrecognised or missing code keeps the pre-existing
    Chinese output rather than guessing.
    """
...
def use_language(builder: Any, language: Optional[str]) -> None:
    """Set the scaffolding language on a ContextBuilder (or a test stand-in).

    Callers use this instead of the constructor so a substitute class without
    a ``language`` parameter still works; the code is normalized here.
    """
    builder._language = _resolve_context_language(language)


def _label(key: str, language: str) -> str:
    variants = _LABELS[key]
    return variants.get(language) or variants[_DEFAULT_CONTEXT_LANGUAGE]
...
def _format_analysis_age(created_at, language: str = _DEFAULT_CONTEXT_LANGUAGE) -> str:
    ...
    if total_seconds < 3600:
        return _label("minutes_ago", language).format(n=int(total_seconds // 60))
    return _label("hours_ago", language).format(n=int(total_seconds // 3600))
...
class ContextBuilder:
    ...
    # Class-level fallback so instances built via `ContextBuilder.__new__`
    # (bypassing `__init__` — several existing tests do this to avoid a real
    # DB connection) still have a valid `self._language`, defaulting to the
    # pre-existing Chinese scaffolding.
    _language: str = _DEFAULT_CONTEXT_LANGUAGE

    def __init__(self, language: Optional[str] = None):
        """``language`` selects the rendered block headers/labels.

        None (or an unsupported code) keeps the pre-existing Chinese
        scaffolding — see ``_resolve_context_language``. This is the fixed
        text around the data, never the data values themselves.
        """
        self._db = DatabaseConnector()
        self._aia: dict = {}
        self._language = _resolve_context_language(language)
    ...
    def _build_investor_profile_section(self, detail: str = "summary") -> str:
        ...
        lines: list[str] = [_label("investor_profile_header", self._language)]
        philosophy: dict = {}
        ...
        try:
            ...
            if row:
                if row[0]:
                    lines.append(_label("investor_label", self._language).format(name=row[0]))
                if row[1]:
                    try:
        ...
        if detail == "summary":
            if goal:
                lines.append(_label("goal_label", self._language).format(value=goal))
            if risk_tolerance:
                lines.append(_label("risk_tolerance_label", self._language).format(value=risk_tolerance))
        else:  # detailed or full
            if goal:
                lines.append(_label("goal_label", self._language).format(value=goal))
            if horizon:
                lines.append(_label("horizon_label", self._language).format(value=horizon))
            if risk_tolerance:
                lines.append(_label("risk_tolerance_label", self._language).format(value=risk_tolerance))
            if core_weakness:
                lines.append(_label("core_weakness_label", self._language).format(value=core_weakness))
        ...
        if detail == "full" and portfolio_structure:
            lines.append(_label("allocation_logic_label", self._language).format(value=portfolio_structure))
    ...
    def _format_ai_insights_section(self, rows: list[tuple[Any, ...]], detail: str) -> str:
        ...
        lines = [_label("ai_insights_header", self._language), ""]
    ...
    def build_portfolio_context(
        ...
        try:
            ...
            lines.append(_label("portfolio_header", self._language))
            if top_level_rows:
                ...
                lines.append(_label("no_holdings_data", self._language))
            ...
            if top_level_rows:
                ...
                if drift_rows:
                    lines.append(_label("largest_drift_header", self._language))
                    for row in drift_rows:
                        ...
                        lines.append(
                            _label("drift_line", self._language).format(
            ...
            lines.append(_label("portfolio_unavailable", self._language))
        ...
        if detail in ("detailed", "full"):
            try:
                ...
                if asset_rows:
                    lines.append(_label("holdings_detail_header", self._language))
                    total_value = sum(float(row.get("market_value") or 0.0) for row in asset_rows) or 1.0
                    lines.append(_label("holdings_table_header", self._language))
                    lines.append("|------|-----------|--------------|----------|-------|")
    ...
    def _build_performance_context(self, include_non_rebalanceable: bool) -> str:
        ...
        lines = [_label("performance_header", self._language)]
        lines.append(
    ...
    def build_market_context(self, detail: str = "summary") -> str:
        ...
        lines: list[str] = [_label("market_header", self._language)]
        try:
            if detail == "summary":
                row = self._db.execute(
                    """
                ...
                if rows:
                    lines.append(_label("raw_market_cache_header", self._language))
    ...
    def build_strategy_context(self, timeframe: str = "30d") -> str:
        ...
        if strategy_path:
            ...
            if md_files:
                sections: list[str] = [_label("strategy_header", self._language)]
        ...
        if memo_rows:
            sections_db: list[str] = [_label("strategy_header", self._language)]
            for memo_date, title, strategic_bias, key_directives, content in memo_rows:
                ...
                if not body:
                    ...
                    body = (
                        ...
                        if directives else _label("no_body", self._language)
                    )
    ...
    def build_transactions_context(self, timeframe: str = "14d", detail: str = "summary") -> str:
        ...
        detail: 'summary' | 'detailed' | 'full'
        """
        lines: list[str] = [_label("transactions_header", self._language)]
        try:
            ...
            if detail == "summary":
                lines.append(_label("tx_table_header_summary", self._language))
                lines.append("|------|------|------|------|------|------|")
                ...
                lines.append(_label("tx_table_header_detailed", self._language))
                lines.append("|------|------|------|------|------|------|------|")
                ...
                lines.append(_label("tx_table_header_full", self._language))
                lines.append("|------|------|------|------|------|------|------|------|------|------|")
    ...
    def build_realtime_context(self) -> str:
        ...
        try:
            ...
            if not results:
                return ""
            lines = [_label("realtime_header", self._language)]
            for item in results:
    ...
    def build_valuation_context(self) -> str:
        ...
        if not rows:
            return ""
        lines = [_label("valuation_header", self._language)]
        lines.append(_label("valuation_note", self._language))
        lines.append("")
        lines.append(_label("valuation_table_header", self._language))
        lines.append("|------|------|---------|---------|------|---------|")
        for r in rows:
    ...
    def build_technical_context(self, detail: str = "summary") -> str:
        ...
        for asset_code, asset_name, technical_signals, created_at in analysis_rows:
            try:
                ...
                summary_line = (
                    f"- {asset_code}({asset_name or asset_code}) "
                    f"{_format_analysis_age(created_at, self._language)}: "
                    f"{trend_status} | RSI={rsi_value:.1f}({rsi_status}) | "
                    f"{_label('technical_score', self._language).format(value=signal_score)}"
                )
                if detail == "summary":
                ...
                detailed_line = (
                    f"{summary_line} | MACD={macd_status} | "
                    f"{_label('technical_volume', self._language).format(value=volume_status)}"
                )
                if detail == "detailed":
                ...
                if support_levels:
                    ...
                    full_line += f" | {_label('technical_support', self._language).format(value=support_str)}"
                if resistance_levels:
                    resistance_str = ", ".join(str(level) for level in resistance_levels)
                    full_line += f" | {_label('technical_resistance', self._language).format(value=resistance_str)}"
        ...
        return _label("technical_header", self._language) + "\n".join(lines)
...
def render_context(
    ...
    ``language`` only selects the top-level "no data" fallback string below —
    the tier blocks themselves already carry whatever language the
    ``context_builder`` was constructed with (``ContextBuilder(language=...)``).
    Callers should pass the same resolved language to both so the fallback
    matches the rest of the rendered text.
    """
    resolved_language = _resolve_context_language(language)
    ...
    if context_blocks:
        return "\n\n".join(context_blocks)
    return _label("context_unavailable_fallback", resolved_language)
