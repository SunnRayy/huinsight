...
from src.services.ai_advisor.context_builder import ContextBuilder, render_context, build_cross_check_context, use_language
...
class ReviewGenerator:
    ...
    def generate_review(
        ...
        if reviewed_context_text is not None:
            ...
            cb = ContextBuilder()
            use_language(cb, lang)
            context_section = render_context(cb, context_config, language=lang)
...
def propose_memo_updates(
    ...
    # 4. Parse proposals (LLM returns a JSON array). Prefer content_json: with
    # expect_json=True the client has already stripped ```json fences and repaired
    # the JSON, which a bare json.loads on the raw content would reject.
    proposals: list[dict] = []
    try:
        parsed = response.content_json
        raw = parsed if isinstance(parsed, (list, dict)) else response.content
