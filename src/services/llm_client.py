...
def llm_keys_configured() -> bool:
    """True if at least one provider API key is present in the environment.

    Cheap and side-effect free (no config or DB access), so routes can check it
    before building context or calling a model.
    """
...
class LLMClient:
    ...
    def _sampling_kwargs(self, model: str) -> dict:
        ...
        Current Claude models (Sonnet 5 and later) reject a non-default
        ``temperature`` with a 400, which the fallback loop would swallow as a
        model failure — so it is omitted for Anthropic models.
        """
    ...
    def _max_tokens_for(self, model: str) -> int:
        ...
        Current Claude models think adaptively by default and thinking counts
        toward ``max_tokens``, so a cap sized for a thinking-off model truncates
        the JSON. Anthropic models get at least 16000.
        """
        if model.partition("/")[0] == "anthropic":
            return max(self._max_tokens, 16000)
