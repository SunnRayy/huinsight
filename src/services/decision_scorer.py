...
def match_trades_to_insights(db: Any, trade_id: int | None = None) -> int:
    ...
    rows = db.execute(
        ...
        WHERE (
          ...
          AND {display_scope}
          -- Never relabel a row that is verified without a verdict: the reader
          -- provenance ('imported'/NULL) is what exempts it from integrity
          -- check trade_log_verdict_consistency (Rule B v2). Relabeling it to
          -- 'memo' or a model name would manufacture a violation — which a
          -- plain GET of /decisions/scorecard did on any DB with a memo
          -- mentioning an imported trade's ticker (Round 5).
          AND NOT (
              LOWER(COALESCE(tl.verification_status, '')) = 'verified'
              AND tl.verdict IS NULL
          )
          {trade_filter}
        """,
        params,
